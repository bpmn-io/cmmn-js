...
function CmmnTreeWalker(handler) {
  ...
  function registerDi(di) {
    ...
    if (cmmnElement) {
      ...
      if (cmmnElement.di) {
        ...
        if (is(cmmnElement, 'cmmn:TextAnnotation')) {
          textAnnotations.push(cmmnElement);
        }
        else if (is(cmmnElement, 'cmmn:Association')) {
          associations.push(cmmnElement);
        }
      ...
      if (is(di, 'cmmndi:CMMNEdge') && di.sourceCMMNElementRef && di.targetCMMNElementRef) {
  ...
  function handleDefinitions(definitions, diagram) {
    ...
    forEach(textAnnotations, contextual(handleTextAnnotation));
  ...
  function handleCasePlanModel(casePlanModel, context) {
    ...
    handleTextAnnotations(casePlanModel, newCtx);
  ...
  function handleItem(item, context) {
    ...
    if (is(definitionRef, 'cmmn:PlanFragment')) {
      ...
      handleTextAnnotations(item, newCtx);
    }
  ...
  function handleTextAnnotations(container, context) {
    if (container.di && container.di.bounds) {

      var bounds = container.di.bounds;
      var annotations = filter(textAnnotations, function(a) {
        return a.di.bounds.x > bounds.x &&
               a.di.bounds.x < (bounds.x + bounds.width) &&
               a.di.bounds.y > bounds.y &&
               a.di.bounds.y < (bounds.y + bounds.height);
      });

      forEach(annotations, function(a) {
        Collections.remove(textAnnotations, a);
        handleTextAnnotation(a, context);
      });
    }
  }
