...
CmmnRules.prototype.init = function() {
  ...
  this.addRule('connection.create', function(context) {
    ...
    canConnect = self.canConnect(source, target) || self.canReplaceConnectionEnd(source, target, 'target');

    if (canConnect === false) {

      canConnect = self.canConnect(target, source) || self.canReplaceConnectionEnd(target, source, 'source');
  ...
  this.addRule('connection.reconnectStart', function(context) {
    ...
    return self.canConnect(source, target, connection) ||
           self.canReplaceConnectionEnd(context.hover, target, 'source');
  });
  ...
  this.addRule('connection.reconnectEnd', function(context) {
    ...
    return self.canConnect(source, target, connection) ||
           self.canReplaceConnectionEnd(source, context.hover, 'target');
  });
...
CmmnRules.prototype.canReplaceConnectionEnd = function(source, target, side) {
  ...
  if (isSame(side, 'target')) {

    if (isSameParent(source, target) &&
        isHumanTask(getDefinition(source)) &&
        isBlocking(source) &&
        isPlanItem(target) &&
        !isEventListener(target) &&
        !isMilestone(target) &&
        !isSameDefinition(source, target)) {
  ...
  if (isSame(side, 'source')) {

    if (isCriterion(target) &&
        isDiscretionaryItem(source) &&
        !isPlanFragment(source)) {
