...
function PlanItemDefinitionUpdater(eventBus, elementRegistry, cmmnFactory, modeling) {
  ...
  this.movePlanItemDefinitions = function(definitions, allMovedItems, context) {
    ...
    var shapes = toArray(context.shapes || context.shape);
    var topLevelItems = unwrapItems(shapes);
  ...
  /**
   * Iterates the given list of elements and triggers the rearrangment 
  ...
  function movePlanItemDefinitionOfElement(element, context) {
    ...
    if (!context.sameCase) {
      ...
      if (referecingItems.length > movedItems.length) {
        ...
        // (2) if the element is moved to another case and
        // the in the former case there are still some items
        // referencing the same plan item definition, then
        // a new plan item definition is created
    ...
    if (referecingItems) {
      ...
      // if there is no referencing items, then it could be that
      // the element does is not drawn on the canvas. Then the
      // most upper stage element (casePlanModel) is fetched and used 
      // as the parent.
  ...
  /**
   * Returns for each given definition the list items
...
/**
 * Returns the common parents stage for given elements.
 *
...
/**
 * Returns parent stages of the given element.
 *
 * @param {ModdleElement} element
 *
 * @param {Array<ModdleElement>} the parent stages
 */
...
/**
 * Returns parent stage of the given element.
 *
 * @param {ModdleElement} element
 *
 * @param {ModdleElement} the parent stage
 */
