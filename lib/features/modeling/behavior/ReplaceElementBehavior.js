...
function ReplaceElementBehavior(
  ...
  this.postExecuted('elements.move', LOW_PRIORITY, function(context) {
    ...
    if (!elements.length) {
      var topLevelElements = context.closure && context.closure.topLevel;
      if (topLevelElements) {
        elements = flatten(values(topLevelElements));
      }
    }
  ...
  this.preExecute('connection.reconnectStart', HIGH_PRIORITY, function(context) {

    var connection = context.connection,
        source = context.source || context.newSource,
        target = context.target || connection.target;

    if (!cmmnRules.canConnect(source, target, connection)) {

      var canReplace = cmmnRules.canReplaceConnectionEnd(source, target, 'source');
      if (canReplace && canReplace.replacements) {

        var replacements = self.replaceElements([ source ], canReplace.replacements, {
          select: false
        });
  ...
  this.preExecute('connection.reconnectEnd', HIGH_PRIORITY, function(context) {

    var connection = context.connection,
        source = context.source || connection.source,
        target = context.target || context.newTarget;

    if (!cmmnRules.canConnect(source, target, connection)) {

      var canReplace = cmmnRules.canReplaceConnectionEnd(source, target, 'target');

      if (canReplace && canReplace.replacements) {
        var replacements = self.replaceElements([ target ], canReplace.replacements, {
          select: false
        });
