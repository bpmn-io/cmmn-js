...
ElementFactory.prototype.createCmmnElement = function(elementType, attrs) {
  ...
  var elementAttrs = assign({}, attrs || {}),
      businessObject = elementAttrs.businessObject;

  if (!businessObject) {
    if (!elementAttrs.type) {
    ...
    if (elementAttrs.name) {
      assign(props, {
        name: elementAttrs.name
      });
    }

    businessObject = this._cmmnFactory.create(elementAttrs.type, props);
  }
  ...
  if (elementType === 'connection') {
    ...
    elementAttrs.type = 'cmmndi:CMMNEdge';
  }

  if (attrs.definitionType) {
    var definition = this._cmmnFactory.create(elementAttrs.definitionType);
    businessObject.definitionRef = definition;
  }

  if (elementAttrs.sentryRef) {
    businessObject.sentryRef = elementAttrs.sentryRef;
  }
  ...
  if (elementAttrs.isCollapsed) {
    businessObject.di.isCollapsed = elementAttrs.isCollapsed;
  }

  if (elementAttrs.isPlanningTableCollapsed) {
    businessObject.di.isPlanningTableCollapsed = elementAttrs.isPlanningTableCollapsed;
  }

  if (elementAttrs.isStandardEventVisible) {
    businessObject.isStandardEventVisible = elementAttrs.isStandardEventVisible;
  }

  if (elementAttrs.isBlocking !== undefined) {
    businessObject.definitionRef.isBlocking = elementAttrs.isBlocking;
  }

  if (elementAttrs.autoComplete) {
  ...
  return this.baseCreate(elementType, elementAttrs);
};
