...
CmmnUpdater.prototype.updateAttachment = function(context) {
  var shape = context.shape,
      ...
      host = shape.host,
      businessObjectHost = host && host.businessObject;
...
CmmnUpdater.prototype.updateParent = function(element, oldParent) {
  ...
  if (!is(businessObject, 'cmmn:Criterion')) {
  ...
  this.updateDiParent(businessObject.di || businessObject, parentDi);
};
...
CmmnUpdater.prototype.updateSemanticParent = function(businessObject, newParent) {
  ...
  if (isAny(businessObject, [ 'cmmndi:CMMNEdge', 'cmmn:DiscretionaryItem', 'cmmn:OnPart' ])) {
    return;
  } else
  ...
  if (is(businessObject, 'cmmn:Artifact')) {
    newParent = newParent && getParent(newParent, 'cmmn:Definitions');
...
CmmnUpdater.prototype.updateConnection = function(context) {
  ...
  if (is(connection, 'cmmn:Association')) {
  ...
  if (sourceAttr && businessObject[sourceAttr] !== newSource) {
    businessObject[sourceAttr] = newSource;
  }

  if (targetAttr && businessObject[targetAttr] !== newTarget) {
    businessObject[targetAttr] = newTarget;
  }
